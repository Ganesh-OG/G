...
```json
{
  ...
  "paragraphs": ["This heading is clickable."]
}
```
...
```json
{
  ...
  "folder": "Projects",
  "file": "dashboard.png",
  "href": "https://example.com/full-image",
  "alt": "Dashboard screenshot"
}
```
...
You can repeat blocks as many times as needed.

```json
{
  "blocks": [
    { "type": "text", "title": "Overview", "text": "Intro" },
    { "type": "image", "location": "local", "folder": "assets/images", "file": "img1.png" },
    { "type": "code", "title": "Script 1", "code": "Write-Host 'One'" },
    { "type": "code", "title": "Script 2", "code": "Write-Host 'Two'" },
    { "type": "quote", "title": "Note", "text": "Extra note" }
  ]
}
```
...
The viewer supports any number of blocks in any order.
...
1. Copy `Template.json` as a starter.
2. Convert the content into JSON.
3. Save it as something like `assets/json/Bulk_AD_User_Creation.json`.
4. Update the admin row:
   - `project_type = file`
   - `project_file_name = Bulk_AD_User_Creation.json`
5. The popup will load `project-view.html` and render the JSON from the `assets/json/` folder.

## Shared Files

- [`project-view.html`](./project-view.html)
- [`assets/json/project-document.js`](./assets/json/project-document.js)
- [`assets/json/project-document.css`](./assets/json/project-document.css)
- [`components/project-popup.js`](./components/project-popup.js)
- [`components/projects.js`](./components/projects.js)

## Migration Note

The old per-project `.HTML` files have been removed. New project content should be added only as `.json` files and opened through the shared viewer.
